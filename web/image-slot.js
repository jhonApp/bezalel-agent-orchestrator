...
(() => {
  ...
  const LOCAL_STATE_KEY = 'bezalel-agent-platform.image-slots.v1';
  ...
  function load() {
    ...
    loadP = fetch(STATE_FILE)
      ...
      .then((j) => {
        // The standalone FastAPI preview cannot expose the editor's sidecar
        // writer. Keep avatars persistent in this browser in that mode.
        const persisted = (j && typeof j === 'object') ? j : readLocalState();
        // Merge: sidecar loses to any in-memory change that raced ahead of
        // the fetch (drop or clear) so neither is clobbered by hydration.
        if (persisted && typeof persisted === 'object') {
          const merged = Object.assign({}, persisted, slots);
          // A framing-only write that raced ahead of hydration must not
          // drop a user image that's only on disk — inherit u from the
          // sidecar for any in-memory entry that lacks one.
          for (const k in slots) {
            if (merged[k] && !merged[k].u && persisted[k]) {
              merged[k].u = typeof persisted[k] === 'string' ? persisted[k] : persisted[k].u;
            }
          }
  ...
  function flushNow() {
    ...
    if (!w) {
      try { window.localStorage.setItem(LOCAL_STATE_KEY, JSON.stringify(slots)); } catch (e) {}
      return;
    }
